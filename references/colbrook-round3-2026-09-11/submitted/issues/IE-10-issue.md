...
[IE-10](https://github.com/ajt60gaibb/OpenProblemsInNLA/blob/main/eigenvalues-and-inverse-problems/IE-10/README.md), the main mathematical target. Statement checked on 11 September 2026 against the repository's 10 September audit.

### Correction or result

**Proposed status: Solution claimed.** Theorem 1 below asserts
$$
\mathbb E\,\kappa_V(H_k)\leq17n^2k,
\qquad
\Pr\{\kappa_V(H_k)\leq1700n^3\}\geq0.99.
$$
The same constants apply to every $n\geq3$ and $2\leq k<n$.

This uses exactly the complex-sphere random starting vector and its actual cyclic-shift Krylov space, not an independently Haar-distributed subspace. It concerns the infimum of the spectral-norm eigenbasis condition number, as in the entry. There is no finite-precision algorithm claim and no claim for arbitrary nonnormal input matrices.

Section 5 independently gives a weaker sufficient bound, with $C=2{,}000{,}000$ and $c=5$, before the expectation argument improves the constants to $C=1700$ and $c=3$. The full displayed repository target is claimed, but the proof has not received independent review.

### Evidence

The self-contained manuscript is included in full below. Main locator: Theorem 1; key identities: (4), (6), and (7); independent probability proof: Proposition 4; root-locus estimate: Lemma 5; final expectation argument: Section 7.

The matching PDF is named `IE-10-proposed-resolution.pdf`. Reproducible supporting code is supplied separately in `verification-support.zip`; tests do not establish the theorem by themselves.
...
No difficulty or importance change is requested on the strength of an unreviewed proof claim. Preserve the original problem statement and ID. Any historical rating or catalogue change should follow mathematical review and the repository procedure.

## Self-contained manuscript

### 1. Claim and scope

This manuscript proposes an affirmative resolution of repository problem **IE-10**, with exactly its complex-sphere starting-vector model.[^repo] The original workshop question specifically mentions the cyclic shift; a counterexample for general nonnormal inputs does not address this case.[^workshop]

**Theorem 1 (cyclic shift).** Let $n\geq3$, let $2\leq k<n$, and let $C_n$ be the cyclic shift on $\mathbb C^n$. Draw $b$ uniformly from the complex unit sphere. Let $Q$ have orthonormal columns spanning
$$
...
We work first with arbitrary distinct points $\zeta_1,\ldots,\zeta_n$ on the unit circle. Write
$$
d=\min_{i\ne j}|\zeta_i-\zeta_j|>0,
$$
and let $w_1,\ldots,w_n$ be independent exponential random variables of mean one. On the sample space $\mathbb C^{\{\zeta_1,\ldots,\zeta_n\}}$, use
$$
\langle f,g\rangle_w=\sum_{j=1}^n w_j\overline{f(\zeta_j)}g(\zeta_j).
$$
Let $\mathcal P_{k-1}$ be the evaluated polynomials of degree less than $k$, let $P_w$ be its orthogonal projection, and set
$$
...
$$
The coefficient representation of $H_w$ is $G^{-1}F$, with coefficient inner product $x^*Gy$.
...
$$
It is an antiunitary involution. For a polynomial of degree less than $k$, it reverses and conjugates the coefficients, so it preserves $\mathcal P_{k-1}$. Hence $J$ commutes with $P_w$. Directly,
$$
JM_zJ=M_z^*,\qquad JH_wJ=H_w^*. \tag{4}
$$
Suppose $\lambda_i$ is simple and $r_i$ is a unit right eigenvector of $H_w$. Then $\ell_i=Jr_i$ is a unit left eigenvector, meaning $H_w^*\ell_i=\overline{\lambda_i}\ell_i$. Put
$$
\alpha_i=\langle\ell_i,r_i\rangle_w,\qquad
\kappa_i=\frac{1}{|\alpha_i|},\qquad
p_{ij}=w_j|r_i(\zeta_j)|^2.
$$
Simplicity gives $\alpha_i\ne0$. The number $\kappa_i$ is the norm of the rank-one spectral projector. Also
$$
\sum_jp_{ij}=1,\qquad
...
Differentiate the generalized eigenvalue equation $Fr_i=\lambda_iGr_i$. Multiplication by the generalized left eigenvector cancels the derivative of $r_i$, giving
$$
...
*Proof.* Each summand in $\alpha_i=\sum_jw_j\overline{\ell_i(\zeta_j)}r_i(\zeta_j)$ has modulus $p_{ij}$. The reverse triangle inequality implies
$$
|\alpha_i|\geq2\max_jp_{ij}-1.
$$
If $\kappa_i>2$, then $\max_jp_{ij}<3/4$. At most one node can lie at distance strictly less than $d/2$ from $\lambda_i$. Consequently at least one quarter of the probability mass in (5) is at distance at least $d/2$, and
$$
...
$$
where the $\kappa_i$ are the norms of its rank-one spectral projectors.

*Proof.* Use orthonormal ambient coordinates. Let $R$ have unit right eigenvectors as columns. The $i$th row of $R^{-1}$ has norm $\kappa_i$: it is the dual left eigenvector with pairing one. Define $D=\operatorname{diag}(\sqrt{\kappa_1},\ldots,\sqrt{\kappa_k})$ and $V=RD$. Then
$$
...
$$
Because $k<n$, the other $n-1$ positive-weight nodes span the polynomial space, so $G_0\succ0$. By the rank-one determinant identity, or multilinearity of the determinant,
$$
...
For positive real $t$, every root lies in the closed unit disk. Except at the finitely many positive zeros of $\Delta$, the roots admit locally analytic labels. All sums over the roots used below are independent of the chosen labeling.
...
*Proof.* Put $\varepsilon=\eta/(16nk)$. For one conditional pencil, call a positive real $t_0$ bad if $t_0\leq2\varepsilon$ or if it is within distance $2\varepsilon$ of a complex zero of $\Delta$. There are at most $2k-2$ such zeros, counted with multiplicity. A disk of radius $2\varepsilon$ cuts the real line in an interval of length at most $4\varepsilon$. Since the exponential density is bounded by one, the conditional bad probability is at most
$$
...
$$
For cyclic nodes, $d\geq4/n$; the bound dominates $2k$ and proves (10). $\square$
...
\end{aligned} \tag{14}
$$
where we used only $te^{-t}\leq1$. Exceptional conditionings have probability zero. Sum (14) over $j$, and then use (7)–(8) and Tonelli's theorem:
$$
...
1. OpenProblemsInNLA. [IE-10: Conditioning of a random Krylov compression of a cyclic shift](https://github.com/ajt60gaibb/OpenProblemsInNLA/blob/main/eigenvalues-and-inverse-problems/IE-10/README.md). Problem statement and audit update of 10 September 2026. Accessed 11 September 2026.

2. Noah Amsel et al. [*Linear Systems and Eigenvalue Problems: Open Questions from a Simons Workshop*](https://arxiv.org/html/2602.05394v3). arXiv:2602.05394v3, 21 August 2026. Section 3.3, Problem 3.5 and its update. This source identifies the question and scope; it is not the source of the proposed proof.

[^repo]: Source 1, the displayed probability target and starting-vector model.

[^workshop]: Source 2, Section 3.3, Problem 3.5 and its 20 August 2026 update.
