"""Read-only public fork/branch/discussion check for IE-02 and IE-05."""
...
if not GH:
    raise SystemExit('Install/authenticate GitHub CLI or set GH to its path.')
...
PAT = re.compile(r'\bIE-0[25]\b|Jordan.{0,60}GMRES|GMRES.{0,60}Jordan|orthogonal.{0,90}(growth|pivot|extrem)|(?:growth|pivot|extrem).{0,90}orthogonal|2308\.16146|2506\.09687', re.I | re.S)
...
def get_tree(b):
    ...
    selected = [v for p, v in paths.items()
                ...
                (any(pid in p for pid in IDS) or p in ['README.md', 'RESOLVED.md', 'CATALOG.md']
                 or re.search('jordan|gmres|orthogonal|growth', p, re.I))]
...
record = {'checked_utc': datetime.now(timezone.utc).isoformat(), 'ids': IDS,
          ...
          'scope': 'All recursively returned public forks, every public branch head; canonical pages, root indexes, ID-named and Jordan/GMRES/orthogonal/growth-named text documents; matching issues, PR bodies and comments, plus all PR review bodies. Private/deleted/unpublished/unidentifiably named work is outside this bounded check.'}
