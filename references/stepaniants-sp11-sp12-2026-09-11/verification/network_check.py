"""Read-only SP-11/SP-12 public audit. Requires an authenticated gh CLI.

Run from any directory; GH may specify the executable and NETWORK_OUTPUT the
output JSON. Bodies are scanned in memory and only their digests are retained.
A regex match is a review candidate, not an automatic resolution classification.
"""
...
REG = json.loads((REPO_ROOT / 'problem_ids.json').read_text())
OUT = Path(os.environ.get('NETWORK_OUTPUT', 'network-rerun.json'))
PAT = re.compile(r'\bSP-(?:11|12)\b|2601[.]01211|Strong.{0,30}Delta|Delta.{0,100}(conjecture|theorem)|chromatic.{0,120}(nullity|Arnold)|Hall.{0,120}(Delta|graph)', re.I | re.S)
...
def get_tree(b):
    ...
    selected = [v for p, v in paths.items()
                ...
                (any(pid in p for pid in IDS) or p in ['README.md', 'RESOLVED.md', 'CATALOG.md']
                 or re.search('delta|greedegree|arnold|chromatic|graph|min.*rank', p, re.I))]
...
def discussion_for(repo):
    ...
    matches = [{'repo': repo, 'url': x['html_url'], 'title': x.get('title'),
                'body_sha256': hashlib.sha256((x.get('body') or '').encode()).hexdigest(), 'kind': 'issue/PR/comment'} for x in entries
...
def review_for(pair):
    ...
    return len(rows), [{'repo': repo, 'pr': num, 'url': r['html_url'], 'body_sha256': hashlib.sha256((r.get('body') or '').encode()).hexdigest(),
...
record = {'checked_utc': datetime.now(timezone.utc).isoformat(), 'ids': IDS,
          ...
          'branches': branches, 'trees': trees, 'contents': {k: {'utf8_bytes': len(v.encode()), 'sha256': hashlib.sha256(v.encode()).hexdigest()} for k, v in contents.items()}, 'discussion_matches': matches,
          'pr_review_bodies_read': sum(n for n, rs in reviews),
          'pr_review_endpoints_read': len(prs),
          'scope': 'All recursively returned public forks, every public branch head; canonical pages, root indexes, ID-named and delta/greedegree/Arnold/chromatic/graph/minimum-rank-named text documents; matching issues, PR bodies and comments, plus all PR review bodies. Private/deleted/unpublished/unidentifiably named work is outside this bounded check.'}
