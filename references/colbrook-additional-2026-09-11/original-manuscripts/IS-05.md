...
**Partial result for IS-05, not a complete resolution.** The bound proved here implies $\alpha_*\le1/2$, but does not determine $\alpha_*$. This is an AI-generated research proposal, not an independently reviewed publication. No priority claim is made.
...
\]
No construction establishing the matching exponent $1/2$ is supplied here. Theorems 1 and 2 are therefore partial progress, not a solution to the exact-value target. The report should propose **Partially resolved**, or leave **Open** with an updated bounds notice at the maintainers' discretion; it must not propose **Solved** or claim an exact exponent.
