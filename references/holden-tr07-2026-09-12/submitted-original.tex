...
\usepackage{enumitem}
\hypersetup{pdftitle={Random column subsets of fixed-sparsity matrices: an unrestricted theorem},pdfauthor={Research draft for independent mathematical review},pdfsubject={A complete proposed analytic proof of TR-07}}
...
\author{Research draft for independent mathematical review}
...
The accompanying standard-library rational tests check finite identities and small instances, including a counterexample to treating the all-hit conditional law as the ideal law. They are implementation checks, not an exhaustive verification of the universal theorem. This manuscript has not been independently human-refereed or checked in a proof assistant. The exact target and publicly visible repository status were checked on 12 September 2026; complete historical issue coverage and publication priority are not asserted. Nothing has been submitted to the repository.
