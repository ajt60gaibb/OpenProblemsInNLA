"""Read-only public fork/branch/discussion check for MF-02 and IE-26."""
...
IDS = ['MF-02', 'IE-26']
REG = {'MF-02': 'matrix-functions-and-stability/MF-02/README.md', 'IE-26': 'linear-systems-and-elimination/IE-26/README.md'}
OUT = args.output
PAT = re.compile(r'\bMF-02\b|\bIE-26\b|cubic.{0,80}sign|sign.{0,80}cubic|Fourier.{0,80}(stability|perturb|interpol)|Austin.{0,5}Trefethen|2608[.]21960|cubic.{0,40}composition|Cheon.{0,40}Kim', re.I | re.S)
...
def get_tree(b):
    ...
    selected = [v for p, v in paths.items()
                ...
                (any(pid in p for pid in IDS) or p in ['README.md', 'RESOLVED.md', 'CATALOG.md']
                 or re.search(r'cubic|composition|sign[-_ ]?(function|approx)|fourier|interpola|MF-02|IE-26|perturb.*fourier', p, re.I))]
...
record = {'checked_utc': datetime.now(timezone.utc).isoformat(), 'ids': IDS,
          ...
          'pr_review_endpoints_read': len(prs), 'immutable_blob_cache_provenance': cache_provenance, 'new_blob_reads': len(missing),
          'scope': 'All recursively returned public forks, every public branch head; canonical pages, root indexes, MF-02/IE-26-named, cubic/composition/sign-function/Fourier/interpolation-named text documents; matching issues, PR bodies and comments, plus all PR review bodies. Private/deleted/unpublished/unidentifiably named work is outside this bounded check.'}
