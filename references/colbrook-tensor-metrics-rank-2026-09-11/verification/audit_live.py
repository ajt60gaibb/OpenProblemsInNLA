...
tasks=[(name,b,i) for name,bs in data['branches'].items() for b in bs for i in ['TR-17','TR-27']]
def read(t):
 name,b,i=t;p=f'tensor-computations/{i}/README.md'
...
def pr_audit(p):
 ...
 return {'number':n,'title':p['title'],'state':p['state'],'target_files':[f for f in files if re.search(r'TR-(?:17|27)(?:/|\b)',f)],'target_review_mentions':[x.get('body','') for x in texts if re.search(r'TR[-_ ]?(?:17|27)\b',x.get('body',''),re.I)]}
...
data['issue_comment_mentions']=[{'url':x['html_url'],'body':x['body']} for page in pages for x in page if re.search(r'TR[-_ ]?(?:17|27)\b',x['body'],re.I)]
