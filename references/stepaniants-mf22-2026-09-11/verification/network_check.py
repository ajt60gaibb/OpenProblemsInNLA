...
ID = 'MF-22'
parser = argparse.ArgumentParser(description='Read-only public MF-22 branch and discussion audit')
...
id_pattern = re.compile(r'(?<![A-Za-z0-9])MF-22(?![A-Za-z0-9])', re.I)
scope_pattern = re.compile(r'\bMF-22\b|cubic.{0,80}condition|C1.{0,80}condition|spline.{0,80}condition|2608[.]24151', re.I | re.S)
...
def get_tree(branch):
    ...
    selected = [item for path, item in paths.items() if
                (id_pattern.search(path) or path in ('README.md', 'RESOLVED.md', 'CATALOG.md') or
                 re.search(r'spline|schrodinger|schrödinger|conditioning', path, re.I)) and
...
report = {
    ...
    'scope': 'All public forks recursively returned from the network source, every public branch head, canonical MF-22 pages, ID-named and spline/Schrodinger/conditioning-named text documents, root README/RESOLVED/CATALOG files; matching issues, PR bodies, issue comments and PR inline-review comments in every repository. Private, deleted, unpublished and unidentifiably named work is outside this check.',
