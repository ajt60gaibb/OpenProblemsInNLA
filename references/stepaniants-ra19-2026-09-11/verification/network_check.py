...
parser.add_argument('--output',type=Path,required=True,help='Explicit path for a new read-only audit snapshot; existing evidence is not overwritten unless selected.')
...
# Keep only target-specific discussion metadata and body hashes, not unrelated
# profile fields or full admission-PR discussion text.
for entry in record['discussion_matches']:
    ...
    for field in ('comments','reviews'):
        if field in entry:
            entry[field]=[{'url':x.get('html_url',x.get('url')),
                           'created_at':x.get('created_at',x.get('submitted_at')),
                           'body_sha256':hashlib.sha256((x.get('body') or '').encode()).hexdigest(),
                           'bytes':len((x.get('body') or '').encode())}
