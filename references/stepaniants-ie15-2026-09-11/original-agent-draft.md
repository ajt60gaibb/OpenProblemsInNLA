...
The standard-library Python script `ie15_certificate.py` checks both rational witnesses: nonsingularity, every selected pivot's row and column inequalities, all active matrices, and the attained growth. It uses `fractions.Fraction` throughout. These finite checks supplement the analytic upper-bound proof; they are not a substitute for it.
