"""Read-only public fork/branch/discussion check for MF-06."""
...
IDS = ['MF-06']
REG = {'MF-06': 'matrix-functions-and-stability/MF-06/README.md'}
OUT = args.output
PAT = re.compile(r'\bMF[-_ ]?0[567]\b|2311[.]18633|2504[.]17505|joint.{0,40}spectral|spectral.{0,20}radius|nonresonan|Barabanov|exterior.{0,60}(power|radius|bound)|pointwise.{0,60}(Lipschitz|Hölder|Holder)', re.I | re.S)
...
def get_tree(b):
    ...
    selected = [v for p, v in paths.items()
                ...
                (any(pid in p for pid in IDS) or p in ['README.md', 'RESOLVED.md', 'CATALOG.md']
                 or re.search(r'MF[-_ ]?0[567]|JSR|joint.?spectral|marginal|product.?bound|nonresonan|Barabanov|exterior|Lipschitz', p, re.I))]
...
for blob_sha, blob_text in contents.items():
    raw = blob_text.encode('utf-8')
    assert hashlib.sha1(b'blob ' + str(len(raw)).encode() + b'\0' + raw).hexdigest() == blob_sha, 'Immutable blob content hash mismatch'
for b in branches:
    b['statuses'] = {}
    for pid in IDS:
...
record = {'repository_metadata': repo_metadata, 'checked_utc': datetime.now(timezone.utc).isoformat(), 'ids': IDS,
          ...
          'scope': 'All recursively returned public forks, every public branch head; canonical pages, root indexes, MF-05/06/07-, JSR-, joint-spectral-, marginal-, product-bound-, nonresonance-, Barabanov-, exterior-, or Lipschitz-named text documents; matching issues, PR bodies and comments, plus all PR review bodies. Private/deleted/unpublished/unidentifiably named work is outside this bounded check.'}
