"""Read-only public fork/branch/discussion check for FR-12."""
...
IDS = ['FR-12']
REG = {'FR-12': 'frames-and-matrix-designs/FR-12/README.md'}
OUT = args.output
PAT = re.compile(r'\bFR-12\b|1808[.]07222|S0963548321000377|Hadamard.{0,100}(enumerat|count|doubl|matching|log|Ferber)|Ferber.{0,60}(Jain|Zhao)', re.I | re.S)
...
def get_tree(b):
    ...
    selected = [v for p, v in paths.items()
                ...
                (any(pid in p for pid in IDS) or p in ['README.md', 'RESOLVED.md', 'CATALOG.md']
                 or re.search(r'hadamard|doubl|matching|ferber|FR-12', p, re.I))]
...
record = {'repository_metadata': repo_metadata, 'checked_utc': datetime.now(timezone.utc).isoformat(), 'ids': IDS,
          ...
          'scope': 'All recursively returned public forks, every public branch head; canonical pages, root indexes, FR-12-named, Hadamard/doubling/matching/Ferber-named text documents; matching issues, PR bodies and comments, plus all PR review bodies. Private/deleted/unpublished/unidentifiably named work is outside this bounded check.'}
