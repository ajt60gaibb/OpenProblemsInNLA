...
ID = 'RA-10'
parser = argparse.ArgumentParser(description='Read-only public RA-10 branch and discussion audit')
...
id_pattern = re.compile(r'(?<![A-Za-z0-9])RA-10(?![A-Za-z0-9])', re.I)
scope_pattern = re.compile(r'\bRA-10\b|constant.loss.{0,50}nuclear|nuclear.{0,50}transfer|unordered.{0,50}operator.monotone', re.I | re.S)
...
def get_tree(branch):
    ...
    selected = [item for path, item in paths.items() if
                (id_pattern.search(path) or path in ('README.md', 'RESOLVED.md', 'CATALOG.md') or
                 re.search(r'nuclear|operator.monotone', path, re.I)) and
...
report = {
    ...
    'scope': 'All public forks recursively returned from the network source, every public branch head, canonical RA-10 pages, ID-named and nuclear/operator-monotone-named text documents, root README/RESOLVED/CATALOG files; matching issues, PR bodies, issue comments and PR inline-review comments in every repository. Private, deleted, unpublished and unidentifiably named work is outside this check.',
