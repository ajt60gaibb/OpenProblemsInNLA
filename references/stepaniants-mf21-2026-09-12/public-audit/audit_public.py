"""Read-only public fork/branch/discussion check for MF-21."""
...
IDS = ['MF-21']
REG = {'MF-21': 'matrix-functions-and-stability/MF-21/README.md'}
OUT = args.output
PAT = re.compile(r'\bMF-21\b|1710[.]05243|2111[.]07196|Conjecture\s*8[.]4|Toeplitz.{0,90}(higher.order|threshold|erratic|expans)|Barrera.{0,60}(B.ttcher|Grudsky|Maximenko)', re.I | re.S)
...
def get_tree(b):
    ...
    selected = [v for p, v in paths.items()
                ...
                (any(pid in p for pid in IDS) or p in ['README.md', 'RESOLVED.md', 'CATALOG.md']
                 or re.search(r'toeplitz|higher.?order|eigenvalue|barrera|grudsky|MF-21', p, re.I))]
...
record = {'repository_metadata': repo_metadata, 'checked_utc': datetime.now(timezone.utc).isoformat(), 'ids': IDS,
          ...
          'scope': 'All recursively returned public forks, every public branch head; canonical pages, root indexes, MF-21-named, Toeplitz/higher-order/eigenvalue/Barrera/Grudsky-named text documents; matching issues, PR bodies and comments, plus all PR review bodies. Private/deleted/unpublished/unidentifiably named work is outside this bounded check.'}
