...
for blob, content in sorted(d['contents'].items()):
    records.append({'git_blob_sha': blob, 'sha256': hashlib.sha256(content.encode()).hexdigest(), 'bytes': len(content.encode()), 'paths': sorted(paths.get(blob, [])), 'mentions_target_or_primary_id': bool(re.search(r'MF-21|1710[.]05243|2111[.]07196', content, re.I))})
heads = []
...
sanitized = {
    ...
    'conclusion': 'Automated retrieval complete. Human assessment of the matching material is recorded separately; this is a bounded public-network snapshot, not a proof of global novelty.'
}
