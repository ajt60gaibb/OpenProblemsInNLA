"""Bounded read-only IE-26 public-branch and discussion eligibility check."""
...
CAN='linear-systems-and-elimination/IE-26/README.md'
...
def read_head(head):
    ...
    selected=[x for x in tree['tree'] if x['type']=='blob' and
              (x['path']==CAN or re.search(r'IE[-_]?26|perturbed.*Fourier|Fourier.*stability',x['path'],re.I))
...
def discussions(repo):
    q=urllib.parse.quote('"IE-26" repo:'+repo+' in:title,body')
...
record={'checked_utc':datetime.now(timezone.utc).isoformat(),'repositories':sorted(repos),
        ...
        'scope':'All recursively returned public forks and branch heads; every canonical or IE26/perturbed-Fourier-named text file; IE-26-matching issue/PR titles and bodies plus their full comments/reviews. Bounded public check; private, deleted, unpublished, and unidentifiably named work excluded.'}
