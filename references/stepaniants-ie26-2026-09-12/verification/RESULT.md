# IE-26: a proof candidate for both perturbed Fourier stability bounds

George Stepaniants  
Department of Computing and Mathematical Sciences  
California Institute of Technology, Pasadena, California, USA

Research manuscript, 12 September 2026. Prepared with AI assistance. **Independent review pending.** This document is a complete proof candidate for the original bundled target; no canonical status or public repository has been changed. The mathematical argument is analytic, not an inference from numerical experiments.
...
## 8. Sources, originality, eligibility, and verification status

The two exact targets were checked against the retained canonical statement, the 2017 Austin–Trefethen paper, and Austin's thesis. Chen–Lin–Zhang's August 2026 preprint retains a logarithm in the second bound and explicitly leaves the $L^\infty$ interpolation conjecture open. Its comparison proof contains $\varepsilon^{-1}m^{4\varepsilon}$; optimizing that expression cannot itself remove the logarithm. The present proof instead uses the positive-real function (3) for the first bound and local weak distribution estimates for the second.

The only external analytic inequality beyond elementary complex analysis used in the proposed proof is the periodic Hilbert transform's weak $(1,1)$ bound. Its exact applicability to the smooth boundary values used here was checked in [4], Theorem 12.1, printed p.67. All perturbed-grid product, residue, localization, and matrix estimates are supplied above.

The fresh public audit at **2026-09-12T01:36:52.722173+00:00** checked five public repositories and 40 branch heads. Existing IE-26 canonical pages all remained Open. The only matching discussion was the admission PR111. The bounded search for later Austin–Trefethen results and arXiv:2608.21960 revisions found no full resolution. Private evidence is in `public-network.json`, SHA-256 `2fd9a18c2fae1029e4fcef36cb6232922ec78f872ad8137f22021b3fdec91098`. This observation does not replace a final eligibility check before any publication. The raw audit may contain unrelated discussion metadata and must be sanitized before public archiving.

This manuscript has not yet received an independent mathematical review, peer review, or formal verification. In particular, its full-resolution claim is a candidate conclusion pending the requested independent adversarial audit. No finite numerical scan is a premise of the proof. The originating agent developed the argument; the coordinating agent supplied an overlapping first-bound derivation before this source was frozen and is therefore recorded as a contributing/coordinating check, not an independent final reviewer. A separate agent must perform the full independent audit. The private script `check_identities.py` checks only three deterministic numerical instances of the residue and Fourier/cardinal identities; its output is supplementary and is not an exact certificate or a premise of the proof.
