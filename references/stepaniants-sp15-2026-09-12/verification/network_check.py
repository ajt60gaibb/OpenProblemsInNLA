"""Read-only public fork/branch/discussion check for SP-15."""
...
GH = os.environ.get('GH', shutil.which('gh') or 'gh')
ROOT = 'ajt60gaibb/OpenProblemsInNLA'
IDS = ['SP-15']
REPO_ROOT = Path(__file__).resolve().parents[3]
REG = json.loads((REPO_ROOT / 'problem_ids.json').read_text())
OUT = Path(os.environ.get('SP15_AUDIT_OUTPUT', 'sp15-network-recheck.json')).resolve()
PAT = re.compile(r'\bSP-15\b|super.?identical|unitary.classes|finite.unitary|jdn085|2109[.]14472', re.I | re.S)
...
def get_tree(b):
    ...
    selected = [v for p, v in paths.items()
                ...
                (any(pid in p for pid in IDS) or p in ['README.md', 'RESOLVED.md', 'CATALOG.md']
                 or re.search('super.?identical|pseudospectr|unitary|SP-15', p, re.I))]
...
record = {'checked_utc': datetime.now(timezone.utc).isoformat(), 'ids': IDS,
          ...
          'branches': branches, 'trees': trees, 'content_sha256': {sha: hashlib.sha256(value.encode()).hexdigest() for sha, value in contents.items()},
          'discussion_matches': [{k: v for k, v in match.items() if k != 'body'} | {'matched_body_sha256': hashlib.sha256((match.get('body') or '').encode()).hexdigest()} for match in matches],
          'pr_review_bodies_read': sum(n for n, rs in reviews),
          'pr_review_endpoints_read': len(prs),
          'scope': 'All recursively returned public forks, every public branch head; canonical pages, root indexes, ID-named and super-identical/pseudospectrum/four-mean-named text documents; matching issues, PR bodies and comments, plus all PR review bodies. Private/deleted/unpublished/unidentifiably named work is outside this bounded check.'}
