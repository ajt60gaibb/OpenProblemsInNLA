...
cache=Path(__file__).resolve().parent;data=json.loads((cache/'eligibility-live.json').read_text());repo='ajt60gaibb/OpenProblemsInNLA'
def gh(*args):return json.loads(subprocess.check_output(['gh',*args],encoding='utf-8'))
tasks=[(name,b,i) for name,bs in data['branches'].items() for b in bs for i in ['RA-02','RA-03']]
def read(t):
 name,b,i=t;p=f'randomized-and-low-rank-approximation/{i}/README.md'
...
def pr_audit(p):
 ...
 return {'number':n,'title':p['title'],'state':p['state'],'target_files':[f for f in files if re.search(r'RA-0[23](?:/|\b)',f)],'target_review_mentions':[x.get('body','') for x in texts if re.search(r'RA[-_ ]?0?[23]\b',x.get('body',''),re.I)]}
...
data['issue_comment_mentions']=[{'url':x['html_url'],'body':x['body']} for page in pages for x in page if re.search(r'RA[-_ ]?0?[23]\b',x['body'],re.I)]
...
print(json.dumps({'branch_targets_checked':len(data['branch_targets']),'not_open':[x for x in data['branch_targets'] if x['status']!='Open'],'pr_target_hits':[x for x in data['pr_file_and_review_audit'] if x['target_files'] or x['target_review_mentions']],'issue_comment_hits':data['issue_comment_mentions']},indent=2))
