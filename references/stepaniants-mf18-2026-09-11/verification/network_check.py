...
ID = 'MF-18'
parser = argparse.ArgumentParser(description='Read-only public MF-18 branch and discussion audit')
...
id_pattern = re.compile(r'(?<![A-Za-z0-9])MF-18(?![A-Za-z0-9])', re.I)
scope_pattern = re.compile(r'\bMF-18\b|imaginary.{0,50}rank|Green.{0,50}rank|rank.{0,50}Green', re.I | re.S)
...
def get_tree(branch):
    ...
    selected = [item for path, item in paths.items() if
                (id_pattern.search(path) or path in ('README.md', 'RESOLVED.md', 'CATALOG.md') or
                 re.search(r'nano|green|riccati|imaginary', path, re.I)) and
...
report = {
    ...
    'scope': 'All public forks recursively returned from the network source, every public branch head, canonical MF-18 pages, ID-named and nano/Green/Riccati/imaginary-named text documents, root README/RESOLVED/CATALOG files; matching issues, PR bodies, issue comments and PR inline-review comments in every repository. Private, deleted, unpublished and unidentifiably named work is outside this check.',
