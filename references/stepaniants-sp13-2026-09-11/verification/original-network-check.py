"""Read-only public fork/branch/discussion check for SP-13."""
...
IDS = ['SP-13']
REG = {'SP-13': 'eigenvalues-and-inverse-problems/SP-13/README.md'}
OUT = Path(__file__).with_name('public-network.json')
PAT = re.compile(r'\bSP-13\b|trace.norm|1808[.]05555|nla[.]2286|perturbations.of.Hermitian', re.I | re.S)
...
def get_tree(b):
    ...
    selected = [v for p, v in paths.items()
                ...
                (any(pid in p for pid in IDS) or p in ['README.md', 'RESOLVED.md', 'CATALOG.md']
                 or re.search('trace.norm|spectral.distribution|NHperturb|SP-13', p, re.I))]
...
record = {'checked_utc': datetime.now(timezone.utc).isoformat(), 'ids': IDS,
          ...
          'scope': 'All recursively returned public forks, every public branch head; canonical pages, root indexes, ID-named and trace-norm/spectral-distribution/NHperturbation-named text documents; matching issues, PR bodies and comments, plus all PR review bodies. Private/deleted/unpublished/unidentifiably named work is outside this bounded check.'}
