"""Read-only public fork/branch/discussion check for SP-13 and SP-15."""
...
IDS = ['SP-13', 'SP-15']
REG = {'SP-13': 'eigenvalues-and-inverse-problems/SP-13/README.md', 'SP-15': 'eigenvalues-and-inverse-problems/SP-15/README.md'}
OUT = args.output
PAT = re.compile(r'\bSP-(13|15)\b|trace.norm|1808[.]05555|nla[.]2286|perturbations.of.Hermitian|super.?identical|unitary.classes|finite.unitary|jdn085|2109[.]14472', re.I | re.S)
...
def get_tree(b):
    ...
    selected = [v for p, v in paths.items()
                ...
                (any(pid in p for pid in IDS) or p in ['README.md', 'RESOLVED.md', 'CATALOG.md']
                 or re.search('trace.norm|spectral.distribution|NHperturb|SP-(13|15)|super.?identical|pseudospectr|unitary', p, re.I))]
...
record = {'checked_utc': datetime.now(timezone.utc).isoformat(), 'ids': IDS,
          ...
          'scope': 'All recursively returned public forks, every public branch head; canonical pages, root indexes, SP-13/SP-15-named, trace-norm, spectral-distribution, NHperturbation, super-identical, pseudospectrum and unitary-named text documents; matching issues, PR bodies and comments, plus all PR review bodies. Private/deleted/unpublished/unidentifiably named work is outside this bounded check.'}
