...
The manuscript claims an algorithm using
\(O(\varepsilon^{-2}[1+\log(nK)])\) exact matrix-vector queries, with
success probability at least \(0.997\), for every input in the displayed
model. It supplies both eigenvalue-location guarantees using the given
condition bound \(K\) and finite exact arithmetic between queries. This
is a query-count claim; it does not bound total runtime, bit complexity
