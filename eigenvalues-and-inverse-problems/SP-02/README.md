...
Let $`A`$ be a self-adjoint, possibly unbounded operator on a separable complex Hilbert space, with $`\sigma(A)=\sigma\cup\Sigma`$ for nonempty closed sets satisfying $`d=\mathop{\mathrm{dist}}\nolimits(\sigma,\Sigma)>0`$. Let $`P=E_A(\sigma)`$. Suppose a bounded self-adjoint perturbation $`V`$ is off-diagonal relative to this decomposition:
