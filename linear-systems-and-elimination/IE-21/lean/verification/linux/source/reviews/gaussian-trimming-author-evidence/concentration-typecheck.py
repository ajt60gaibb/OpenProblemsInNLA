...
build=Path(tempfile.mkdtemp(prefix='nla-ie21-concentration-milestone-'))
...
log_text=(evidence/'concentration-typecheck.log').read_text()
axioms=re.findall(r"'([^']+)' depends on axioms: \[([^\]]*)\]",log_text)
assert len(axioms)==6,axioms
allowed={'propext','Classical.choice','Quot.sound'}
assert all(set(a.strip() for a in closure.split(','))<=allowed for _,closure in axioms)
receipt={'milestone':'exact generic five-tail concentration of fractional trimming',
 'scope':'generic concentration helpers with original finite constants; spherical pointwise wrapper and full campaign gates not included',
 ...
 'log_sha256':hashlib.sha256((evidence/'concentration-typecheck.log').read_bytes()).hexdigest()}
(evidence/'concentration-author-evidence.json').write_text(json.dumps(receipt,indent=2)+'\n')
print('Evidence saved; 6 public axiom closures allowed. Build:',build,flush=True)
