...
build=Path(tempfile.mkdtemp(prefix='nla-ie21-empirical-milestone-'))
...
files=['NLA/IE21/Definitions.lean','NLA/IE21/GaussianTrimming.lean','NLA/IE21/PopulationTrimming.lean','NLA/IE21/QuantileTrimming.lean',
 'NLA/IE21/MatrixSemantics.lean','NLA/IE21/FiniteTrimming.lean','NLA/IE21/EmpiricalTrimming.lean',
 'reviews/gaussian-trimming-author-evidence/EmpiricalAxioms.lean']
records=[]
with (evidence/'empirical-typecheck.log').open('w') as log:
...
log_text=(evidence/'empirical-typecheck.log').read_text()
axioms=re.findall(r"'([^']+)' depends on axioms: \[([^\]]*)\]",log_text)
assert len(axioms)==3,axioms
allowed={'propext','Classical.choice','Quot.sound'}
assert all(set(a.strip() for a in closure.split(','))<=allowed for _,closure in axioms)
receipt={'milestone':'deterministic bounded threshold and fractional empirical trimming error',
 'scope':'partial IE-21 helper author evidence; no additional frozen main target completed here',
 ...
 'log_sha256':hashlib.sha256((evidence/'empirical-typecheck.log').read_bytes()).hexdigest()}
(evidence/'empirical-author-evidence.json').write_text(json.dumps(receipt,indent=2)+'\n')
print('Evidence saved; 3 public axiom closures allowed. Build:',build,flush=True)
