...
build=Path(tempfile.mkdtemp(prefix='nla-ie21-quantile-milestone-'))
...
files=['NLA/IE21/Definitions.lean','NLA/IE21/GaussianTrimming.lean','NLA/IE21/PopulationTrimming.lean','NLA/IE21/QuantileTrimming.lean',
 'reviews/gaussian-trimming-author-evidence/QuantileAxioms.lean']
records=[]
with (evidence/'quantile-typecheck.log').open('w') as log:
...
log_text=(evidence/'quantile-typecheck.log').read_text()
...
receipt={'milestone':'atom-robust quantile existence/bound, fractional selector and exact dual attainment',
 ...
 'log_sha256':hashlib.sha256((evidence/'quantile-typecheck.log').read_bytes()).hexdigest()}
(evidence/'quantile-author-evidence.json').write_text(json.dumps(receipt,indent=2)+'\n')
