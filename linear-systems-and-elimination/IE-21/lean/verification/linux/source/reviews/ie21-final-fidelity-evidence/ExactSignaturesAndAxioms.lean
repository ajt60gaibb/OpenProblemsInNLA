...
example : ∀ {Ω : Type*} [MeasurableSpace Ω]
  ...
  μ.map A = matrixLaw m n := @NLA.IE21.independent_row_transport
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
  ...
  (matrixLaw m n).real (GoodEvent θ m n t ε δ)ᶜ ≤ finiteFailure m n t ε δ := @NLA.IE21.uniform_trim_concentration
