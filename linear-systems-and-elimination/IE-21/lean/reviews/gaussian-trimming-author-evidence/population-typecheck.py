...
build=Path(tempfile.mkdtemp(prefix='nla-ie21-population-milestone-'))
...
files=['NLA/IE21/Definitions.lean','NLA/IE21/GaussianTrimming.lean','NLA/IE21/PopulationTrimming.lean',
 'reviews/gaussian-trimming-author-evidence/PopulationAxioms.lean']
records=[]
with (evidence/'population-typecheck.log').open('w') as log:
...
log_text=(evidence/'population-typecheck.log').read_text()
axioms=re.findall(r"'([^']+)' depends on axioms: \[([^\]]*)\]",log_text)
assert len(axioms)==11,axioms
allowed={'propext','Classical.choice','Quot.sound'}
assert all(set(a.strip() for a in closure.split(','))<=allowed for _,closure in axioms)
receipt={'milestone':'population dual bounds, law transport, coupling Lipschitz, quantile attainment and Gaussian dual equality',
 ...
 'log_sha256':hashlib.sha256((evidence/'population-typecheck.log').read_bytes()).hexdigest()}
(evidence/'population-author-evidence.json').write_text(json.dumps(receipt,indent=2)+'\n')
print('Evidence saved; 11 public axiom closures allowed. Build:',build,flush=True)
