...
RUN = BASE / 'canonical-runs/IE04-35034399633'
ART = RUN / 'artifacts/lean-IE-04'
LOG = ART / 'verify-20260915T231116Z-4006'
REPO = Path('/Users/georgestepaniants/Research/OpenProblemsInNLA')
WORK = Path('/private/tmp/nla-lean-next-ie04-worktree')
REL = 'linear-systems-and-elimination/IE-04/lean'
PROJECT = WORK / REL
COMMIT = '026b3e5534a4d6e15ebffb85318c2ff031df32bf'
PRIOR = BASE / 'reviews/IE04-inequalities-full-source-referee'
...
assert run['id'] == fetch['run'] == 35034399633
...
verify = next(j for j in jobs['jobs'] if j['id']==104600154206)
...
assert len(arts['artifacts'])==1 and artifact['id']==10423196054
assert artifact['workflow_run']['id']==35034399633 and artifact['workflow_run']['head_sha']==COMMIT
archive = RUN/'lean-IE-04.zip'
...
raw=(RUN/'job-104600154206.log').read_text(encoding='utf-8-sig')
...
for f,h in prior['source_files'].items():
    now=inputs[f]
...
assert config==result['config'] and len(config['theorem_names'])==21
...
assert len(re.findall(r'^warning: Challenge\.lean:.*declaration uses `sorry`',comp,re.M))==21
...
for name in config['theorem_names']:
    ...
    assert re.search(r'^#assert_trust kernel '+r'(?:NLA\.IE04\.)?'+re.escape(name.removeprefix('NLA.IE04.'))+r'\s*$',solution,re.M)
...
for f in ['run.json','jobs.json','artifacts.json','FETCH-IDENTITY.json',
          'job-104600154206.log','job-104600059442.log','lean-IE-04.zip']:
...
out={'reviewer':'/root/next_inequalities','run_id':35034399633,'commit':COMMIT,
     'job_id':104600154206,'artifact_id':10423196054,'archive_sha256':ziphash,
     'all_project_Git_inputs':inputs,'all_project_inputs_match_worktree':True,
     'archive_members':members,'raw_job_sha256':sha((RUN/'job-104600154206.log').read_bytes()),
