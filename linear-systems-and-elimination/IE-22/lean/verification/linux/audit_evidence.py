#!/usr/bin/env python3
"""Audit retained full IE-22 Linux evidence; does not rerun Lean or review mathematics."""
...
publication = "c45f9ccef20a2fa5cc4b988e93d4173dab853362"
guest_commit = "3ee64b33ae1c626b3c7ffa1c4e561b6055623bb9"
...
assert sha(base / "input.tar") == input_receipt["archive_sha256"] == \
    "eb5dc854bca9aa8025c0db4af0f3e0538ffba4d262b68dcaf08616dbb62d6931"
assert (base / "input.tar").stat().st_size == input_receipt["archive_bytes"] == 1597440
...
assert result["project"] == input_receipt["project_path"] == "linear-systems-and-elimination/IE-22/lean"

source = base / "source"
hashes = {str(p.relative_to(source)): sha(p) for p in source.rglob("*") if p.is_file()}
assert len(hashes) == len(input_receipt["input_sha256"]) == preparation["tracked_input_count"] == 394
...
assert provenance["host_preparation_issues_before_verification"] == 0
...
assert len(names) == len(set(names)) == preparation["selected_theorem_count"] == 20
...
challenge_names = ["NLA.IE22." + n for n in re.findall(
...
assert sha(source / "reviews/package-source-freeze.json") == input_receipt["package_source_freeze_sha256"] == "2a07dedc9ca3774cd9fc5a760f421dd47f105cc257c770f401ae73513dea1226"
assert len(freeze["source_sha256"]) == 57
for name, digest in freeze["source_sha256"].items():
    assert hashes[name] == digest, name
correctness_dir = source / "reviews/ie22-final-correctness-evidence"
fidelity_dir = source / "reviews/ie22-final-fidelity-evidence"
correctness = json.loads((correctness_dir / "final-source-review-receipt.json").read_text())
correctness_inspection = json.loads((correctness_dir / "package-inspection-receipt.json").read_text())
fidelity = json.loads((fidelity_dir / "source-receipt.json").read_text())
assert sha(correctness_dir / "package-inspection-receipt.json") == correctness["package_inspection_receipt_sha256"]
assert correctness_inspection["source_files"] == fidelity["source_sha256"] == freeze["source_sha256"]
assert correctness["package_source_freeze_sha256"] == fidelity["package_source_freeze_sha256"] == input_receipt["package_source_freeze_sha256"]
assert correctness["authored_IE22_statement_or_proof"] is False
assert correctness["verdict"] == "APPROVE" and correctness["mathematical_modules_read_and_freshly_compiled"] == 47
assert correctness["selected_full_frozen_types_passed"] == 20
assert fidelity["independent_nonauthor_of_boundary_proofs_and_packaging"] is True
assert fidelity["verdict"].startswith("APPROVE source")
assert fidelity["all_47_modules_read_and_freshly_rebuilt"] is True
assert fidelity["all_20_exact_full_signatures_and_standard_three_axiom_closures_pass"] is True
...
expected_modules = {p[:-5].replace("/", ".") for p in hashes if p.startswith("NLA/") and p.endswith(".lean")}
assert len(expected_modules) == 47
built_modules = set(re.findall(r"Built (NLA\.[A-Za-z0-9_.]+) \(", proof))
assert built_modules == expected_modules, (expected_modules - built_modules, built_modules - expected_modules)
...
for declarations, module in exports:
    assert [n for n in declarations.split(", ") if n.startswith("NLA.IE22.")] == names, module
...
print("PASS: exact input/evidence archives, all394 source files and distinct commit provenance")
...
print("PASS: all47 project proof modules freshly rebuilt")
print("PASS: all20 reviewed declarations exported separately and matched by Comparator")
print("PASS: authentic LeanCert kernel assertions compiled; every exported theorem uses only standard3 axioms")
print("PASS: actual Lean kernel accepted the complete IE-22 solution; verifier EXIT0")
