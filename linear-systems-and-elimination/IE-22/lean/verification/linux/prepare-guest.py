...
assert receipt['project_path']=='linear-systems-and-elimination/IE-22/lean'
...
assert names==receipt['theorem_names'] and len(names)==len(set(names))==20
assert set(config['permitted_axioms'])=={'propext','Classical.choice','Quot.sound'}
challenge_names=['NLA.IE22.'+n for n in re.findall(r'^theorem ([A-Za-z_][A-Za-z_0-9]*)',(project/'Challenge.lean').read_text(),re.M)]
...
git('init','-q','-b','verification-ie22')
git('add','--',receipt['project_path'])
git('-c','user.name=George Stepaniants','-c','user.email=','commit','-q','-m','Freeze unchanged reviewed IE-22 input from publication '+receipt['publication_commit'])
