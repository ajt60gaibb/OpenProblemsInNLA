...
The selected matrix and supremum bounds supply every certificate input
from the proved spectral and probability results.
...
/-- The exact manuscript finite bound, supplied by an actual spectral restriction
and a nonempty Gaussian certificate event. -/
theorem deterministic_finite_bound (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
  ...
  have hn : 1 ≤ n := by omega
  have hd : 1 ≤ n-r := by omega
  obtain ⟨J,hop,hrows,_⟩ := spectral_projection m n r hm hr A hA
  have hmin := (projection_semantics θ hθ m n (n-r) hn hd A J).2.2
  have hprob := (projection_good_event_bound θ hθ m (n-r) r hm hd hr.1
    (projectedMatrix A J) hrows hop δ hδ).2
  exact deterministic_bound_of_projected_probability θ hθ m n r hm hr δ hδ hfail
    A (projectedMatrix A J) hmin hprob

/-- Transfer to the literal canonical supremum, including its unsquared form. -/
theorem supremum_finite_bound (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
  ...
  exact supremum_bound_of_all_matrices θ hθ m n hm (by omega) (deterministicBound θ n r δ)
    (fun A hA => deterministic_finite_bound θ hθ m n r hm hr δ hδ hfail A hA)
