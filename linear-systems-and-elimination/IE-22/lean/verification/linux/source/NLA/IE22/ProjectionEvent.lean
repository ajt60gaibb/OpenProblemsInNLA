...
The selected final target supplies all intermediate moment inputs from
the proved exact Gaussian variance and energy modules.
...
/-- Assembly with explicit moment inputs; the selected theorem below supplies
them from the proved Gaussian modules. -/
...
/-- Operator control of the full row-pair covariance sum, without row independence.
Initial proof supplied by the independent spectral-module author. -/
theorem row_inner_square_sum_le {m d : ℕ} (B : Mat m d) :
    (∑ i, ∑ j, (inner ℝ (matrixRow B i) (matrixRow B j)) ^ 2) ≤
      operatorNorm B ^ 2 * ∑ i, ‖matrixRow B i‖ ^ 2 := by
  rw [Finset.mul_sum]
  apply Finset.sum_le_sum
  intro i _
  have h := (matrixMap B).le_opNorm (matrixRow B i)
  have hs := (sq_le_sq₀ (norm_nonneg _) (mul_nonneg (norm_nonneg _) (norm_nonneg _))).2 h
  simpa [EuclideanSpace.real_norm_sq_eq, matrixMap_eq_inner_row,
    real_inner_comm, operatorNorm, mul_pow] using hs

lemma gaussian_projectedEnergy_variance_le {m d r : ℕ} (hm : 1 ≤ m) (hd : 1 ≤ d)
    (B : Mat m d) (hrows : ∀ i, ‖matrixRow B i‖ ≤ 1)
    (hop : operatorNorm B^2 ≤ (m : ℝ)/((r : ℝ)+1)) :
    Var[projectedEnergy B;stdGaussian (Space d)] ≤ 2/((r : ℝ)+1) := by
  have hmpos : (0:ℝ)<m := by exact_mod_cast (show 0<m by omega)
  have hm0 : (m:ℝ) ≠ 0 := ne_of_gt hmpos
  have htrace : Matrix.trace (gramMatrix B*gramMatrix B) ≤ operatorNorm B^2*Matrix.trace (gramMatrix B) := by
    rw [gram_sq_trace_eq_row_inner_sum,gram_trace_eq_row_sum]
    exact row_inner_square_sum_le B
  rw [(gaussian_energy_moments m d hm hd B).2.2]
  calc
    _ ≤ 2*(operatorNorm B^2*Matrix.trace (gramMatrix B))/(m:ℝ)^2 := by gcongr
    _ ≤ 2*(((m:ℝ)/((r:ℝ)+1))*(m:ℝ))/(m:ℝ)^2 := by
      gcongr
      · exact gram_trace_nonneg B
      · exact gram_trace_le_rows B hrows
    _ = _ := by field_simp

theorem projection_good_event_bound (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    (m d r : ℕ) (hm : 1 ≤ m) (hd : 1 ≤ d) (_hr : 1 ≤ r) (B : Mat m d)
    (hrows : ∀ i, ‖matrixRow B i‖ ≤ 1)
    (hop : operatorNorm B ^ 2 ≤ (m : ℝ) / ((r : ℝ) + 1))
    (δ : ℝ) (hδ : 0 < δ ∧ δ < 1) :
    MeasurableSet (ProjectionGood θ B δ) ∧
    (stdGaussian (Space d)).real (ProjectionGood θ B δ)ᶜ ≤ projectionFailure θ d r δ := by
  exact projection_good_event_of_energy_variance θ hθ m d r hm hd B hrows hop δ hδ.1
    (gaussian_projectedEnergy_memLp B) (gaussian_projectedEnergy_mean_le_one hm B hrows)
    (gaussian_projectedEnergy_variance_le hm hd B hrows hop)
    (fun t ht => gaussian_objective_variance θ m d hm hd B t ht)
