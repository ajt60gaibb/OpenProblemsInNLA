...
expected=json.loads(author_receipt.read_text())['source_sha256']; expected.update({f:digest(root/f) for f in ['NLA/IE22/DeterministicSchedule.lean','NLA/IE22/AsymptoticConclusion.lean']})
...
assert len(deps)==31
build=Path(tempfile.mkdtemp(prefix='nla-ie22-asymptotic-referee-',dir='/private/tmp'))
...
sources=['NLA/IE22/Definitions.lean','NLA/IE22/SupremumSemantics.lean','NLA/IE22/SphericalRealization.lean','NLA/IE22/DeterministicSchedule.lean','NLA/IE22/AsymptoticConclusion.lean','reviews/asymptotic-referee-infrastructure-evidence/HelperTypesAndAxioms.lean']
...
names=[n for f in sources[4:5] for n in re.findall(r'^(?:theorem|lemma) ([A-Za-z0-9_]+)',(root/f).read_text(),re.M)]
