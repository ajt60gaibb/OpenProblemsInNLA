...
build=Path('/private/tmp/nla-ie22-variance-tensorization-author-build');build.mkdir(exist_ok=True)
...
with (evidence/(sys.argv[1]+'.log')).open('w') as log:
 log.write('IE22 variance tensorization author development build; pinned cached dependencies and unchanged verified IE21 cache; no full IE22 or Linux/LeanCert verification claim.\n')
 for f in sys.argv[2:]:
