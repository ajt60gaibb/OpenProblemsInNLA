...
campaign=Path('/private/tmp/nla-solved-campaign-20260922')
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
expected={'NLA/IE22/Definitions.lean':'206dd0bc8af86b5a67541985a9ba209472cf6788dba7120b8adaf00545fba371','Challenge.lean':'03336766670095f08f74eb9d6a0704ed8812ca9cde31a99d2a2fbc6521a11152','NUMERICAL_TARGETS.md':'31d41813f8e6c7952d5d197e0b2246dfa79dfe8c6ffab56fdb0c64a4d5d4334e','comparator.json':'f625c401bf276a2e93b7e76b2b052555c8686b57590fd7da16fdb56e2840c7a8'}
for p,digest in expected.items():assert sha(root/p)==digest,p
assert json.loads((root/'reviews/draft-stage-receipt.json').read_text())['boundary_sha256']==expected
canonical=campaign/'linear-systems-and-elimination/IE-22/README.md'
manuscript=campaign/'references/colbrook-recovered-2026-09-11/manuscripts/IE-21-22.tex'
statement=canonical.read_text().split('## Problem statement\n',1)[1].split('## Connection to numerical linear algebra',1)[0].strip()
num=(root/'NUMERICAL_TARGETS.md').read_text()
quoted=num[num.index('Fix $`0<\\theta<1`$.'):num.index('## Literal objects and obligatory semantic bridges')].strip()
assert statement==quoted,'Original full problem statement differs'
config=json.loads((root/'comparator.json').read_text())
names=['NLA.IE22.'+name for name in re.findall(r'^theorem (\w+)',(root/'Challenge.lean').read_text(),re.M)]
assert names==config['theorem_names'] and len(names)==20
assert not (root/'Solution.lean').exists()
assert {str(p.relative_to(root)) for p in (root/'NLA/IE22').glob('*.lean')}=={'NLA/IE22/Definitions.lean'}
typecheck=json.loads((evidence/'typecheck-receipt.json').read_text())
assert typecheck['typechecked_source_sha256']=={p:expected[p] for p in ['NLA/IE22/Definitions.lean','Challenge.lean']}
report='''# IE-22 independent preproof statement review
...
| --- | --- |
'''
for p,digest in expected.items():report+=f'| `{p}` | `{digest}` |\n'
report+='''
...
'''
report_path=root/'reviews/statement-referee-infrastructure.md';report_path.write_text(report)
r={'reviewer':'Codex AI /root/infrastructure_audit','completed_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'verdict':'APPROVE exact preproof statements; second independent approval and explicit freeze required before implementation','is_ai_review':True,'authored_new_IE22_boundary':False,'transitive_IE21_authorship':['SphericalLaw','RowLaw','GaussianPolar','GaussianMoments','SphericalMoments','SphericalMGF'],'boundary_sha256':expected,'canonical_sources_sha256':{str(canonical.relative_to(campaign)):sha(canonical),str(manuscript.relative_to(campaign)):sha(manuscript)},'complete_original_statement_text_match':True,'selected_theorem_count':len(names),'theorem_names':names,'all_selected_statements_read':True,'full_manuscript_read':True,'unresolved_mathematical_findings':[],'independent_typecheck':'PASS: Definitions and Challenge EXIT0; exactly20 intentional reference placeholders, no other Lean diagnostics','typecheck_receipt_sha256':sha(evidence/'typecheck-receipt.json'),'author_draft_receipt_sha256':sha(root/'reviews/draft-stage-receipt.json'),'statement_review_sha256':sha(report_path),'evidence_sha256':{p.name:sha(p) for p in sorted(evidence.iterdir()) if p.is_file() and p.name!='receipt.json'},'IE22_proofs_implemented':False,'IE22_Comparator_executed':False,'IE22_LeanCert_executed':False,'publication_packaging_pending':True}
(evidence/'receipt.json').write_text(json.dumps(r,indent=2)+'\n')
print('APPROVE exact20-target preproof boundary')
print('report SHA256',sha(report_path))
