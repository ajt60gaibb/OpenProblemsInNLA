...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    (m n : ℕ) (hm : 1 ≤ m) (hn : 1 ≤ n), (unitRowValues θ m n).Nonempty ∧ BddAbove (unitRowValues θ m n) ∧
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1), 0 ≤ sharpConstant θ ∧ sharpConstant θ ^ 2 = gaussianTrim θ ∧
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    ...
    (J : Space d →ₗᵢ[ℝ] Space n), (∀ g : Space d, matrixMap (projectedMatrix A J) g = matrixMap A (J g)) ∧
...
example : ∀ (m n r : ℕ) (hm : 1 ≤ m) (hr : 1 ≤ r ∧ r < n)
    (A : Mat m n) (hA : UnitRows A), ∃ J : Space (n - r) →ₗᵢ[ℝ] Space n,
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    ...
    (hB : ∀ i, ‖matrixRow B i‖ ≤ 1) (t : ℝ) (ht : 0 ≤ t), Integrable (projectedObjective θ B t) (stdGaussian (Space d)) ∧
...
example : ∀ (θ : ℝ) (m d : ℕ)
    (hm : 1 ≤ m) (hd : 1 ≤ d) (B : Mat m d) (t : ℝ) (ht : 0 ≤ t), MemLp (projectedObjective θ B t) 2 (stdGaussian (Space d)) ∧
...
example : ∀ (m d : ℕ) (hm : 1 ≤ m) (hd : 1 ≤ d) (B : Mat m d), MemLp (projectedEnergy B) 2 (stdGaussian (Space d)) ∧
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    ...
    (hmean : (∑ i, y i) / (m : ℝ) ≤ 2), ∃ t : ℝ, 0 ≤ t ∧ t ≤ truncationScale θ ∧
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    (m : ℕ) (hm : 1 ≤ m) (y : Fin m → ℝ) (s t : ℝ), |trimDual (retainedRows θ m) y s - trimDual (retainedRows θ m) y t| ≤ |s - t| := @NLA.IE22.threshold_lipschitz

#print axioms NLA.IE22.threshold_lipschitz

example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1) (δ : ℝ) (hδ : 0 < δ), ∃ C : Finset ℝ, (∀ t ∈ C, t ∈ Icc 0 (truncationScale θ)) ∧
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    ...
    (δ : ℝ) (hδ : 0 < δ ∧ δ < 1), MeasurableSet (ProjectionGood θ B δ) ∧
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    ...
    (A : Mat m n) (hA : UnitRows A), normalizedDeletion θ A ≤ deterministicBound θ n r δ := @NLA.IE22.deterministic_finite_bound

#print axioms NLA.IE22.deterministic_finite_bound

example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    (m n r : ℕ) (hm : 1 ≤ m) (hr : 1 ≤ r ∧ r < n)
    (δ : ℝ) (hδ : 0 < δ ∧ δ < 1) (hfail : deterministicFailure θ n r δ < 1), extremalValue θ m n ^ 2 ≤ deterministicBound θ n r δ ∧
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1), (∀ᶠ n in atTop, 1 ≤ deletionSchedule n ∧ deletionSchedule n < n ∧
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1), ∃ C : ℝ, 0 < C ∧ ∃ N : ℕ, 1 ≤ N ∧
      ∀ m n : ℕ, 1 ≤ m → N ≤ n →
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    (ε : ℝ) (hε : 0 < ε), ∃ N : ℕ, 1 ≤ N ∧ ∀ m n : ℕ, 1 ≤ m → N ≤ n →
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    ...
    (hfail : finiteFailure m n t ε δ < 1), ∃ A : Mat m n, UnitRows A ∧
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    ...
    (ε : ℝ) (hε : 0 < ε), ∀ᶠ j in atTop, ∃ A : Mat (m j) (n j), UnitRows A ∧
...
example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1)
    (m n : ℕ → ℕ) (hm : ∀ j, 1 ≤ m j) (hn : ∀ j, 1 ≤ n j)
    (hnlim : Tendsto n atTop atTop)
    (hQlim : Tendsto (fun j => (m j : ℝ) / n j) atTop atTop), Tendsto (fun j => extremalValue θ (m j) (n j)) atTop (𝓝 (sharpConstant θ)) := @NLA.IE22.high_aspect_supremum_limit

#print axioms NLA.IE22.high_aspect_supremum_limit

example : ∀ (θ : ℝ) (hθ : 0 < θ ∧ θ < 1), EventualUniformUpper θ (sharpConstant θ) ∧
