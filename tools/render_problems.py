...
def render(source):
    ...
    with tempfile.TemporaryDirectory(prefix=f"nla-{identifier}-") as work:
        ...
        if identifier in {"IE-02", "IE-04", "IE-14", "IE-21", "IE-22", "IV-03", "KE-05", "MF-05", "MF-12", "MF-18", "MF-21", "MF-22", "MI-24", "MI-27", "MI-28", "NM-04", "NR-04", "PF-03", "SP-04", "SP-05", "SP-15", "TR-27"}:
        ...
        if identifier in {
            ...
            tex = re.sub(r"\\subsection\{References?(?:\s+and\s+status\s+check)?\}",
                         lambda m: "\\newpage\n" + m[0], tex, count=1)
        if identifier in {"IE-21", "IE-22", "SP-11", "SP-12"}:
