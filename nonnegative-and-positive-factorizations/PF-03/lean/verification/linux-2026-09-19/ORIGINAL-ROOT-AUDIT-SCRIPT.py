"""Audit actually downloaded PF-03 GitHub artifacts; never runs Lean/Comparator."""
...
E = D / 'verification/PF03-linux-20260919'
PROOF = '9625a76780183040186664100e24e0e90d8fcc7d'
...
assert len(names) == 25 and not config['definition_names']
...
assert len(expected_inputs) == 234
lock = hashlib.sha256(blob('tools/lean/source-lock.json')).hexdigest()
results = []
for kind, run_id, job_id, repository in [
    ('fork', 35424055075, 105846873543, 'sgstepaniants/OpenProblemsInNLA'),
    ('upstream', 35424087832, 105846968481, 'ajt60gaibb/OpenProblemsInNLA')]:
    root = E / kind
    ...
    job = selected[0]
    assert job['id'] == job_id and job['name'] == f'verify (PF-03, {PROJECT})'
    ...
    assert artifact['name'] == 'lean-PF-03' and not artifact['expired']
    assert 'sha256:' + sha(root / 'lean-PF-03.zip') == artifact['digest']
    assert (root / 'lean-PF-03.zip').stat().st_size == artifact['size_in_bytes']
...
report = {'scope': 'Root read-only audit of real downloaded GitHub evidence; no additional Lean or Comparator run.',
          'time': datetime.datetime.now(datetime.timezone.utc).isoformat(), 'verdict': 'PASS',
          'script_sha256': sha(Path(__file__)), 'runs': results,
...
(E / 'ROOT-AUDIT.json').write_text(json.dumps(report, indent=2) + '\n')
print(json.dumps({'verdict': 'PASS', 'actual_GitHub_runs': len(results), 'inputs_per_run': len(expected_inputs), 'contracts_per_run': len(names), 'root_audit_sha256': sha(E / 'ROOT-AUDIT.json')}))
