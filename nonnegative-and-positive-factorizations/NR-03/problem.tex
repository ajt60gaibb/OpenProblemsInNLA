...
on the Nonnegative Rank via Non-Convex Optimization Solvers}}, §6.7 and
Appendix A.4, still states the full-rank conjecture. Searches for its
resolution found none. Sergeev's July 2026
\href{https://arxiv.org/abs/2607.27014}{\emph{Upper bounds for the
monotone rank of the unique disjointness matrix}} concerns ranks of a
\textbf{partial} unique-disjointness matrix with unspecified entries
when \(a^{\mathsf T}b>1\); those bounds do not determine the rank of
this prescribed completion. The open \(n=3\) instance is not counted
separately.
\end{document}
