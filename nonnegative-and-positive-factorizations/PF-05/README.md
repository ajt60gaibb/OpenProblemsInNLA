...
Let $`M\in\mathbb R_+^{p\times q}`$ satisfy $`\mathop{\mathrm{rank}}\nolimits(M)=3`$ and
$`\mathop{\mathrm{rank}}\nolimits_{\rm psd}(M)=2`$, and fix any size-two factorization
$`M_{ij}=\mathop{\mathrm{tr}}\nolimits(A_iB_j)`$. Zero entries of $`M`$ are allowed.

Call a collection of real symmetric matrices $`(E_i,F_j)`$ a feasible linear
direction if

```math
\mathop{\mathrm{tr}}\nolimits(E_iB_j)+\mathop{\mathrm{tr}}\nolimits(A_iF_j)=0\quad\text{for every }i,j
```
