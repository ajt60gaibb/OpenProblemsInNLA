...
2026-09-12}\label{partial-results-2026-09-12}

\textbf{Author:} Sidney Holden, Center for Computational Biology,
Flatiron Institute, Simons Foundation
(\href{https://www.simonsfoundation.org/people/sidney-holden/}{verified
affiliation}). The
\href{https://github.com/ajt60gaibb/OpenProblemsInNLA/blob/main/references/holden-ra04-2026-09-12/RA04_partial_results.pdf}{complete
report} and
\href{https://github.com/ajt60gaibb/OpenProblemsInNLA/blob/main/references/holden-ra04-2026-09-12/src/report.tex}{editable
source} establish the following partial results, with \(m=k'=bt\) and
\(\Delta=\Delta_{k'}^{(b)}\).

Theorem 5.3 proves all three requested guarantees for every admissible
input with

\[q=O\!\left(\frac{t\log(2m/\Delta)+\log(n/(\delta\varepsilon))}{\sqrt\varepsilon}\right).\]

This has an extra \(t\log m\) term. The requested iteration order holds
when \(\Delta\leq1/m\) (Corollary 5.4), when the leading spectrum
consists of exactly \(t\) levels each repeated \(b\) times (Theorem
6.1), when \(t=2\) with arbitrary admissible leading spectra (Theorem
7.1), and at the endpoints \(b=1\) and \(b=k\) (Theorem 6.1 and
Corollary 6.2). Proposition 8.1 gives almost-sure exact optimal
approximation when \(\mathop{\mathrm{rank}}\nolimits(A)=m\) after
\(t+1\) blocks. Theorem 4.1 supplies an alternative all-input bound with
leading term \((m-b+1)\log(3/\Delta)\).

\textbf{Remaining target:} the displayed original bound for arbitrary
growing \(b,t\), nonzero-width leading clusters and \(\Delta>1/m\) is
neither proved nor disproved. The report's sufficient interpolation
estimate (IE) remains unproved. Its raw-monomial conditioning
counterexample (Proposition 9.1) does not refute RA-04, whose algorithm
depends on the Krylov subspace.

A
\href{https://github.com/ajt60gaibb/OpenProblemsInNLA/blob/main/references/holden-ra04-2026-09-12/verification/independent-review.md}{separate
independent Codex agent review} passed the partial arguments and checked
the imported convergence theorem against the primary source. This is
informal AI-agent review, not external human peer review or formal
verification. It does not meet the complete-resolution requirement for
\textbf{Solved}.
\href{https://github.com/ajt60gaibb/OpenProblemsInNLA/blob/main/references/holden-ra04-2026-09-12/README.md}{Submission
