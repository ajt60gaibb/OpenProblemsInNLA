...
\href{https://github.com/ajt60gaibb/OpenProblemsInNLA/blob/main/references/holden-ra14-2026-09-12/package/report.pdf}{\emph{Bounds
and a spectral-to-PCA reduction}}, Theorems 1.1, 1.2 and 5.1, Sections 2
and 6--8. The
\href{https://github.com/ajt60gaibb/OpenProblemsInNLA/blob/main/references/holden-ra14-2026-09-12/README.md}{submission
record} includes verified affiliation, source attribution and
reproducible checks.

The note proves a universal lower bound of \(k\) queries, exact
\(k\)-query complexity under a rank-at-most-\(k\) promise, and hence
\(q_{\mathrm{sp}}=\Theta(n)\) when \(k\ge n/2\), using exact column
recovery for the upper bound. A deterministic spectral-to-PCA
postprocessor on symmetric inputs satisfying the stated gap promises
uses at most \(k\lceil10/\sqrt{\varepsilon}\rceil\) extra products.
Combining this with the cited PCA lower bound yields
\(\Theta(k\log n/\sqrt{\varepsilon})\) when
\(n\ge C_*(k/\varepsilon)^D\) for universal constants. The polynomial
dimension restriction is essential and retained; the exponent is not
optimized or explicitly evaluated.
...
informal Codex AI-agent audit} reviews these partial claims. The
simultaneous finite-parameter characterization outside these regimes
remains unresolved. \textbf{Status remains Partially resolved.} No full
solution, external human peer review or formal verification is claimed;
