...
Reference 1's displayed manuscript is dated August 24, 2026 and retains
the conjecture. It states that \(p\ge2r\) is proved, as is the matrix
case \(r=p=2\); general \(p<2r\) remains beyond the available bounds.
Searches \textit{"tensor" "type-2" "conjecture" "2026"} and
\textit{"tensor" "type-2" conjecture proof 2025 2026} found no full
resolution. Results for independent entries or rank-one summands impose
stronger hypotheses. The \(p=\infty\) endpoint is already covered by the
source's established high-\(p\) regime and is not an additional open
entry.
\end{document}
