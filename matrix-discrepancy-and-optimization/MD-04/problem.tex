...
September 2026, \textbf{Corollary 1.2, p.~2}. It proves the displayed
target with \(C=3\sqrt{2\pi}\) for every allowed \(m,n,t\), including
the smaller sparsities left unresolved by the earlier results. Since a
\(0\)--\(1\) column has squared Euclidean norm equal to its number of
nonzero entries, apply Theorem 1.1 to \(A/\sqrt t\) and multiply the
strict discrepancy bound by \(\sqrt t\). Every column receives a sign.
...
\href{https://github.com/ajt60gaibb/OpenProblemsInNLA/blob/main/matrix-discrepancy-and-optimization/MD-04/solution.md}{proof
by reference}, \textbf{Theorem 2}
...
Stepaniants authors the explanatory note; both the theorem and its
Beck--Fiala corollary are credited to Guo, Fang and Lu.

A separate Codex agent checked the application and substantive source
proof and returned \textbf{PASS}:
...
review}. Verification is independent automated-agent review, not
external human peer review or formal certification. The source remains a
preprint and discloses Odin AI use; the application draft was prepared
with ChatGPT. No case of the original target remains open under the
reviewed result. The original statement and earlier status evidence
remain below, and the ratings above are historical.
