...
```math
\mathop{\mathrm{per}}\nolimits\bigl(tJ_n+(1-t)A\bigr)
\le t\mathop{\mathrm{per}}\nolimits(J_n)+(1-t)\mathop{\mathrm{per}}\nolimits(A)
\qquad\text{for every }t\in[1/2,1]?
```

Here $`\mathop{\mathrm{per}}\nolimits(C)=\sum_{\sigma\in S_n}\prod_{i=1}^n c_{i,\sigma(i)}`$, so $`\mathop{\mathrm{per}}\nolimits(J_n)=n!/n^n`$. Neither symmetry nor positive semidefiniteness is assumed for $`A`$.
