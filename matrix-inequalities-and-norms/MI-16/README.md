...
\operatorname{per}\!\left(U^*\operatorname{diag}(\lambda_1,\ldots,\lambda_n)U\right),
\qquad \mathcal U_n=\{U\in\mathbb C^{n\times n}:U^*U=I_n\},$$

where $\operatorname{per}(C)=\sum_{\sigma\in S_n}\prod_{i=1}^n c_{i,\sigma(i)}$ and $S_n$ is the permutation group. The permanent of these Hermitian positive-semidefinite matrices is real. Compactness ensures the maximum exists. A resolution should express its value from the prescribed eigenvalues for arbitrary order, rather than only bound the objective or restate its optimization definition.
