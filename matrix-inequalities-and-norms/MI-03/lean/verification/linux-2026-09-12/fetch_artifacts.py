"""Read-only retrieval for the independent MI-03 operational audit.
...
selected_jobs = [j for j in jobs if j['name'] in ['select', 'checker-controls']
                 or j['name'].startswith('verify (MI-03,')]
...
selected = [a for a in metadata['artifacts'] if a['name'] in
            ['lean-MI-03', 'lean-checker-controls']]
assert {a['name'] for a in selected} == {'lean-MI-03', 'lean-checker-controls'}
