...
ref=sys.argv[2]
ev=project/'verification'/'author-completion'
...
record={'command':cmd,'exit_code':r.returncode,'elapsed_seconds':round(time.monotonic()-start,3),'log_sha256':hashlib.sha256(r.stdout).hexdigest(),'platform':'macOS arm64, using fresh author prefix; not Linux Comparator'}
...
assert b'PROJECT_DECLARATIONS:' in r.stdout
