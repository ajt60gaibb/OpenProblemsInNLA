"""Retain completed standalone controls while the MI-03 project is still running.
...
print(json.dumps({'scope':'Completed standalone controls only; no MI-03 verdict','run_status_at_retrieval':run['status'],'job':job['id'],'artifact':a['id'],'sha256':hashlib.sha256(archive).hexdigest()}))
