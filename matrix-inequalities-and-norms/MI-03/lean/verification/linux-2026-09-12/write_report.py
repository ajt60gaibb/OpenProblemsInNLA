...
commit=idn['commit'];run_id=idn['run'];project=idn['project'];count=len(idn['input_files']);assert count==173
v=next((out/'artifacts/lean-MI-03').glob('verify-*'));receipt=json.loads((v/'result.json').read_text());assert receipt['input_sha256']=={p:r['sha256'] for p,r in read('preflight.json')['input_files'].items()}
pack=json.loads((out/'source'/project/'verification/root-linux-packaging.json').read_text());freeze=json.loads((out/'source'/project/'reviews/proof-freeze.json').read_text())
job=lambda name:next(j for j in jobs if j['name']==name)
pjob=next(j for j in jobs if j['name'].startswith('verify (MI-03,'))
...
reviews='\n'.join(f"- `{name}`: `{h}`." for name,h in pack['independent_reports'].items())
...
report=f'''# MI-03 independent Linux operational audit - 12 September 2026
...
- Relevant jobs: selection `{job('select')['id']}`, standalone checker
  `{job('checker-controls')['id']}`, and [MI-03 `{pjob['id']}`](https://github.com/sgstepaniants/OpenProblemsInNLA/actions/runs/{run_id}/job/{pjob['id']}).
...
new `{ctrl['fresh_project_directory']}` directory, constrained the environment,
