"""Independent MI-03 operational audit by /root/formal_review_standards.
...
WT = Path('/tmp/nla-lean-mi03-worktree')
COMMIT = '901ba5ffad3b57557b60c7360df67659d8b8aa21'
BASE = 'c0601d8825e9f9e744212c62e6a43fefc1c60a22'
PROJECT = 'matrix-inequalities-and-norms/MI-03/lean'
RUN = 34722618003
V = next((OUT / 'artifacts/lean-MI-03').glob('verify-*'))
...
assert sha(proof_freeze_bytes) == 'fcff9e256a6425853b15def24260b419613a72d9122c0a5bedea4b4cd5f0fd1d'
proof_freeze = json.loads(proof_freeze_bytes)
for rel, record in proof_freeze['files'].items():
    digest = record['sha256']
    target = pack['historical_README_archive'] if rel == 'README.md' else rel
    assert receipt['input_sha256'][target] == digest, rel
for name, digest in pack['independent_reports'].items():
    assert receipt['input_sha256']['reviews/' + name] == digest, name
assert pack['independent_reports']['proof-referee-1.md'] == '7a00100cd2070c759b0dc71c743f9d58428c2ae5bfe41ffda4696efd4003391f'
assert pack['independent_reports']['proof-referee-2.md'] == '129c3b352346d1da8a192a0f4d662eb0a6e9b9f5a7a02b7efc176f2a7a051ba3'
core = ['NLA/MI03/Definitions.lean', 'Challenge.lean', 'NUMERICAL_TARGETS.md',
        'SOURCE_MAP.md', 'NLA/MI03/Modulus.lean', 'NLA/MI03/UpperBound.lean',
        'NLA/MI03/Roots.lean', 'NLA/MI03/Witness.lean', 'NLA/MI03/Sharpness.lean',
        'NLA/MI03/Proof.lean', 'Solution.lean']
ref1 = json.loads((WT / PROJECT / 'reviews/proof-referee-1-evidence/integrity-after.json').read_text())
ref2 = json.loads((WT / PROJECT / 'reviews/proof-referee-2-evidence/integrity-after.json').read_text())
assert ref1['result'] == ref2['status'] == 'PASS'
for rel in core:
    assert ref1['file_hashes'][rel] == ref2['project_files'][rel] == receipt['input_sha256'][rel], rel
...
for manifest_rel, expected in pack['proof_referee_evidence'].items():
    manifest_path = WT / PROJECT / manifest_rel
    assert sha(manifest_path.read_bytes()) == expected['sha256']
    assert manifest_path.stat().st_size == expected['bytes']
    ...
    assert actual == set(bound)
    for rel, r in bound.items():
        p = manifest_path.parent / rel
        assert p.stat().st_size == r['bytes'] and sha(p.read_bytes()) == r['sha256']
assert len(proof_freeze['files']) == pack['frozen_project_inputs'] == 101
assert len(proof_freeze['source_files']) == pack['unchanged_original_sources'] == 8
assert pack['unchanged_nonREADME_inputs'] == 100
config = json.loads((WT / PROJECT / 'comparator.json').read_text())
names = ['modulus_semantics', 'contraction_modulus', 'positive_decomposition',
         'universal_upper_bound', 'root_of_unity_data', 'sharpness_witness',
         'sharp_constant', 'odd_contraction_conjecture']
assert config == receipt['config']
assert config['permitted_axioms'] == ['propext', 'Classical.choice', 'Quot.sound']
assert config['definition_names'] == []
assert config['theorem_names'] == ['NLA.MI03.' + name for name in names] == pack['exports']
...
candidate_manifest = WT / PROJECT / 'verification/candidate-2026-09-12/EVIDENCE-MANIFEST.json'
assert sha(candidate_manifest.read_bytes()) == '446254ffc64f76869fe9ee332e4c27604e9c842e28de3ffa5e485a883b3e0e36'
candidate = json.loads(candidate_manifest.read_text())
for rel, r in candidate['files'].items():
    data = (candidate_manifest.parent / rel).read_bytes()
    assert sha(data) == r['sha256'] and len(data) == r['bytes']
for rel, r in candidate['metadata'].items():
    assert receipt['input_sha256'][rel] == r['sha256']
...
selected = [job for job in jobs if job['name'] in ['select', 'checker-controls']
            or job['name'].startswith('verify (MI-03,')]
...
project_job = next(job for job in selected if job['name'].startswith('verify (MI-03,'))
...
for item in json.loads((OUT / 'artifact-metadata.json').read_text())['artifacts']:
    if item['name'] not in {'lean-MI-03', 'lean-checker-controls'}:
        continue
    ...
    job = project_job if item['name'] == 'lean-MI-03' else next(job for job in selected if job['name'] == 'checker-controls')
    raw = (OUT / f"job-{job['id']}.log").read_text()
...
controls = {}
for label, directory in [('checker-controls', S), ('MI-03 controls', V)]:
...
for module in ['Definitions', 'Modulus', 'UpperBound', 'Roots', 'Witness', 'Sharpness', 'Proof']:
    assert 'Built NLA.MI03.' + module in main
...
assert len(axiom_records) == main.count('depends on axioms:') == proof_freeze['kernel_assertion_count'] == 16
...
assertion_counts = {rel: (WT / PROJECT / rel).read_text().count('#assert_trust kernel ')
                    for rel in ['NLA/MI03/Proof.lean', 'Solution.lean']}
assert assertion_counts == {'NLA/MI03/Proof.lean': 8, 'Solution.lean': 8}
...
implementation_paths = sorted((WT / PROJECT / 'NLA/MI03').glob('*.lean')) + [WT / PROJECT / 'Solution.lean']
...
assert json.loads((WT / PROJECT / 'reviews/proof-referee-1-evidence/inspection-summary.json').read_text())['actual_required_dependencies']
...
for label in ['lean-MI-03', 'lean-checker-controls']:
...
save('identity-verification.json', {'result': 'PASS for actual MI-03 and control jobs',
    ...
    'independent_review_hashes': pack['independent_reports'], 'archives': archives,
...
save('control-verification.json', {'result': 'PASS', 'control_runs': controls,
    ...
    'LeanCert_role': 'Explicit kernel trust auditing of a pure exact proof; no numerical interval certificate. The exact all-k complex roots, CFC moduli, Euclidean operator norms, universal positive decomposition and actual IsLeast/infimum proof are bound to both independent final reviews by the actual verified input set.'})
