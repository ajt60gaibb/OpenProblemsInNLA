...
For every integer $n\ge1$ and positive definite $A,B\in\mathbb C^{n\times n}$ with $\operatorname{tr}(A+B)=1$, set $a=\operatorname{tr}A$, $b=\operatorname{tr}B$. Is
$$
\bigl\|B\log(A+B)-\log(A+B)B\bigr\|_1
\le -a\log a-b\log b
$$
always true? The logarithm is the natural logarithm applied by spectral functional calculus; $\|X\|_1=\operatorname{tr}(X^*X)^{1/2}$. Thus $a,b>0$ and $a+b=1$, and every expression is well-defined.
